...
class JobTracker(Service):
    ...
    def exposed_load_task(self, input_file, output_file, mapper_file, mapper_name, reducer_file, reducer_name, cfg):
        print('begin')
        job = {}
        job['job_id'] = self.get_job_id()
        job['cfg'] = json.loads(cfg)
        job['mapper_num'] = job['cfg']['mapper_num']
        job['reduce_num'] = job['cfg']['reduce_num']
        job['input_file'] = input_file
        job['output_file'] = output_file
        job['mapper_file'] = mapper_file
        job['mapper_name'] = mapper_name
        job['reducer_file'] = reducer_file
        job['reducer_name'] = reducer_name
        job['status'] = 'waiting'
        job['finished_mapper'] = 0
        job['finished_reduce'] = 0
    ...
    def process_task(self, job):
        ...
        for i in range(split_nums):
            ...
            while current_split_size > 0:
                ...
                split_data.append({'blk_no':fat.loc[fat_no]['blk_no'],'blk_size':fat.loc[fat_no]['blk_size'],'host_name':fat.loc[fat_no]['host_name'],'begin_inds':begin_inds,'end_inds':end_inds})
            ...
            if host in task_tracker_hosts:
                shares.append({'job_id':job['job_id'],'input_file':job['input_file'],'host_name':host,'split_no':i,'split_data':split_data})
            else:
                task_track_random_inds = random.randint(0,len(self._task_tracker_hosts)-1)
                shares.append({'job_id':job['job_id'],'input_file':job['input_file'],'host_name':self.task_tracker_hosts[task_track_random_inds],'split_no':i,'split_data':split_data})
        ...
        partition_infos = [[] for i in range(job['reduce_num'])]
        for connection, response in zip(connections, responses):
            partitions = response.value
            for idx, partition in enumerate(partitions):
                partition_info ={'host_name':share['host_name'], 'port':self._task_tracker_ports[0], 'partition':partition}
                partition_infos[idx].append(partition_info)
            connection.close()
     
        print("load the reducer")
        reducer = {'connections':[], 'responses':[]}
        for idx, partition_info in enumerate(partition_infos):
            task_track_random_inds = random.randint(0, len(self._task_tracker_hosts)-1)
            task_track_conn = rpyc.connect(self._task_tracker_hosts[task_track_random_inds], self._task_tracker_ports[0])
            load_reducer = rpyc.async_(task_track_conn.root.load_reducer)
            response = load_reducer(job['reducer_file'], job['reducer_name'], partition_info, job)
            reducer['connections'].append(task_track_conn)
            reducer['responses'].append(response)
            
        # process the response
        print(reducer.keys())
        connections = reducer['connections']
        responses = reducer['responses']
